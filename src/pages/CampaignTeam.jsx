...
const CampaignTeam = () => {
  ...
  const fetchEmployees = async () => {
    try {
      ...
      const url = '/api/employees?team=Campaign&status=Active';
  ...
  const updateTaskInState = (taskId, updateFn) => {
    ...
    setGroups(newGroups);
    localStorage.setItem("campaignGroups", JSON.stringify(newGroups));
  };
  ...
  const handleAssign = async (task, empID) => {
    ...
    try {
      const token = localStorage.getItem('access_token');
      const res = await fetch(`/api/campaign/tasks/${task.id}/assign`, {
        method: 'PUT',
        headers: {
    ...
    if (window.confirm("⚠️ DELETE WARNING:\nThis will remove the task from this team AND delete the data from the Client Page.\n\nAre you sure?")) {
      ...
      const updatedClients = allClients.map(client => {
        if (client.clientID === task.clientID) {
          ...
          // Reset Campaign Data
          if (newClient.Campaign) {
            newClient.Campaign = { ...newClient.Campaign, count: 0, amount: 0, min: 0 };
          }
          return newClient;
        }
        return client;
  ...
  const updateTaskField = (task, field, value) => {
    ...
    // Update local storage for employee tasks if needed (legacy logic support)
  ...
  return (
    <div className="container">
      <h1 style={{ textAlign: 'center', color: '#007bff', marginBottom: '20px' }}>Campaign Team Dashboard</h1>
