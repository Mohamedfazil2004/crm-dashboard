...
const WebsiteTeam = () => {
  ...
  const fetchEmployees = async () => {
    try {
      ...
      const url = '/api/employees?team=Website&status=Active';
      const res = await fetch(url, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      console.log(`[DEBUG] Fetching employees from: ${url}`);
      if (res.ok) {
        const data = await res.json();
        console.log("[DEBUG] Received employees:", data);
        // Include everyone EXCEPT Team Leads and Admins in the assignment list
        const filtered = (data || []).filter(emp => emp.role !== 'Team Lead' && emp.role !== 'Admin');
        console.log("[DEBUG] Filtered employees for dropdown:", filtered);
  ...
  const fetchTasks = async () => {
    try {
      const token = localStorage.getItem('access_token');
      const res = await fetch('/api/website/tasks', {
        headers: {
  ...
  const updateTaskInState = (taskId, updateFn) => {
    ...
    setGroups(newGroups);
    localStorage.setItem("websiteGroups", JSON.stringify(newGroups));
  };
  ...
  const handleAssign = async (task, empID) => {
    ...
    try {
      const token = localStorage.getItem('access_token');
      const res = await fetch(`/api/website/tasks/${task.id}/assign`, {
        method: 'PUT',
        headers: {
  ...
  const updateTaskField = (task, field, value) => {
    ...
    if (assignedEmp) {
      ...
      const updatedEmpTasks = empTasks.map(t => (t.id === task.id) ? { ...t, [field]: value } : t);
  ...
  return (
    <div className="container">
      <h1 style={{ textAlign: 'center', color: '#007bff', marginBottom: '20px' }}>Website Team Dashboard</h1>
      <div className="table-container">
        <table>
          ...
          <tbody>
            {groups.length === 0 ? (
              <tr><td colSpan="6" style={{ textAlign: 'center' }}>No tasks assigned</td></tr>
            ) : (
...
const TaskItemRow = ({ task, employees, onAssign, onDelete, onUpdate, isAdmin, setActiveChatTask, getTaskStatus, getStatusStyles }) => {
  ...
  const getMinutes = (t) => {
    if (t.minutes && Object.keys(t.minutes).length > 0) return getTotal(t.minutes);
    if (t.min) return parseInt(t.min) || 0;
    return 0;
  };
  ...
  return (
    <tr>
      ...
      <td>Web (AT009)</td>
      <td style={{ maxWidth: '200px', fontSize: '13px', color: '#555' }}>{task.description || "-"}</td>
      <td style={{ textAlign: 'center', fontWeight: 'bold', fontSize: '14px' }}>{getTotal(task.count)}</td>
      <td>{getMinutes(task)}</td>
      ...
      <td style={{ maxWidth: '150px', fontSize: '11px', color: '#666', fontStyle: 'italic' }} title={task.employeeRemark}>
        {task.employeeRemark || "-"}
      </td>
      <td>
        <span style={{
