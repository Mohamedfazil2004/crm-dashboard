...
const TelecallerTeam = () => {
  ...
  const fetchEmployees = async () => {
    try {
      ...
      const url = '/api/employees?team=Telecaller&status=Active';
  ...
  const fetchTasks = async () => {
    try {
      const token = localStorage.getItem('access_token');
      const res = await fetch('/api/telecaller/tasks', {
        headers: {
  ...
  const updateTaskInState = (taskId, updateFn) => {
    ...
    setGroups(newGroups);
    localStorage.setItem("telecallerGroups", JSON.stringify(newGroups));
  };
  ...
  const handleAssign = async (task, empID) => {
    ...
    try {
      const token = localStorage.getItem('access_token');
      const res = await fetch(`/api/telecaller/tasks/${task.id}/assign`, {
        method: 'PUT',
        headers: {
    ...
    if (window.confirm("⚠️ DELETE WARNING:\nThis will remove the task from this team AND delete the data from the Client Page.\n\nAre you sure?")) {
      ...
      const updatedClients = allClients.map(client => {
        if (client.clientID === task.clientID) {
          ...
          // Reset Calls Data
          if (newClient.Calls) {
            newClient.Calls = { ...newClient.Calls, count: 0, amount: 0 };
          }
          return newClient;
        }
        return client;
  ...
  const updateTaskField = (task, field, value) => {
    ...
    if (assignedEmp) {
      ...
      const updatedEmpTasks = empTasks.map(t => (t.clientID === task.clientID && t.activityCode === task.activityCode) ? { ...t, [field]: value } : t);
  ...
  return (
    <div className="container">
      <h1 style={{ textAlign: 'center', color: '#d35400', marginBottom: '20px' }}>Telecaller Team Dashboard</h1>
      ...
      <div style={{
        ...
        <div className="table-container">
          <table>
            <thead>
              <tr>
                <th>Active Code</th><th>Work</th>
                <th>Client</th><th>Activity</th><th>Work</th><th>Calls</th><th>Min</th><th>Chat</th><th>Active</th><th>Employee Remark</th><th>Status</th>
                <th>Team Lead Remark</th>
