...
const SeoTeam = () => {
  ...
  const fetchEmployees = async () => {
    try {
      ...
      const url = '/api/employees?team=SEO&status=Active';
  ...
  const updateTaskInState = (taskId, updateFn) => {
    ...
    setGroups(newGroups);
    localStorage.setItem("seoGroups", JSON.stringify(newGroups));
  };
  ...
  const handleAssign = async (task, empID) => {
    ...
    try {
      const token = localStorage.getItem('access_token');
      const res = await fetch(`/api/seo/tasks/${task.id}/assign`, {
        method: 'PUT',
        headers: {
    ...
    if (window.confirm("⚠️ DELETE WARNING:\nThis will remove the task from this team AND delete the data from the Client Page.\n\nAre you sure?")) {
      ...
      const updatedClients = allClients.map(client => {
        if (client.clientID === task.clientID) {
          ...
          // Reset SEO Data
          if (newClient.SEO) {
            newClient.SEO = { ...newClient.SEO, count: 0, amount: 0, min: 0 };
          }
          return newClient;
        }
        return client;
  ...
  const updateTaskField = (task, field, value) => {
    ...
    // Update local storage for employee tasks if needed
    ...
    if (assignedEmp) {
      ...
      localStorage.setItem(empKey, JSON.stringify(updatedEmpTasks)); // Fixed parsing error from original code if it existed
    }
  };

  return (
    <div className="container">
      <h1 style={{ textAlign: 'center', color: '#007bff', marginBottom: '20px' }}>SEO Team Dashboard</h1>
